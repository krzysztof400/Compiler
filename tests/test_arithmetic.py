...
from tests.helpers import compile_source_to_mr, extract_ints, record_koszt
...
def run_program(mr_path: Path, a: int, b: int, request):
    import subprocess
    ...
    record_koszt(request, res.stdout, res.stderr)
...
def test_addition(compiled_program, a: int, b: int, request):
    _, _, s, _ = run_program(compiled_program, a, b, request)
    assert s == a + b, f"Sum mismatch: {s} != {a} + {b} (expected {a + b})"


@pytest.mark.parametrize("a,b", [(12, 8), (21, 14), (13, 5), (100, 3), (0, 1), (7, 7), (10, 0), (5, 2), (0, 0), (9, 4)])
def test_subtraction(compiled_program, a: int, b: int, request):
    _, _, _, t = run_program(compiled_program, a, b, request)
    assert t == max(a - b, 0), f"Difference mismatch: {t} != max({a} - {b}, 0) (expected {max(a - b, 0)})"


@pytest.mark.parametrize("a,b", [(12, 8), (21, 14), (13, 5), (100, 3), (0, 1), (7, 7), (10, 0), (5, 2), (0, 0), (9, 4)])
def test_division(compiled_program, a: int, b: int, request):
    _, q, _, _ = run_program(compiled_program, a, b, request)
    if b == 0:
...
@pytest.mark.parametrize("a,b", [(12, 8), (21, 14), (13, 5), (100, 3), (0, 1), (7, 7), (10, 0), (5, 2), (0, 0), (9, 4)])
def test_modulus(compiled_program, a: int, b: int, request):
    r, _, _, _ = run_program(compiled_program, a, b, request)
    if b == 0:
